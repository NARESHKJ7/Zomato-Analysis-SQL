...
Zomato is an Indian leader in online food ordering and delivery services, with an estimated 58% market share and an exponential growth record in recent years. While the acquisition of Blinkit accelerated the expansion of quick commerce, recent trends indicate a revenue slowdown in core food delivery for FY25 and a profitability dip in Q1 FY26. The delisting of 19,000 restaurants, along with a shift in partner preferences toward Swiggy and ONDC-based platforms such as Digihaat due to rising commission rates, has further intensified these challenges.
...
- The `app_pageviews` table follows a simplified journey of `/home` → `/search` → `/restaurants` → `/menu` → `/cart` → `/thankyou` page (Order Confirmation), although in practice the ordering path may vary. For this analysis, each session is limited to producing at most one order, with no multi-restaurant orders.
...
The UX funnel showed a 12%+ abandonment rate at checkout. Mega and high-value orders dominated (8,708 orders), with Zomato Wallet usage steady at 24% across all order values. Single-item orders were the costliest to serve, with a delivery fee share of GMV at 13.46% compared to 2.82% for big carts, while the overall average delivery fee charged stood at roughly ₹33.4 (inclusive of ₹0 fees for Gold members on orders above ₹199). Engagement peaked between 8-10 PM, though weekends did not deliver the expected surge.
...
- The Zomato Food-Rescue feature has delivered a strong impact, saving 29.18% of would-be losses post-launch. However, total losses still rose from ₹20,994 to ₹25,776, likely due to a higher order volume mix seen above.
...
- “Get Set Zomato” Initiative: Launch a food-on-the-go service, which would require establishing Zomato Express pickup counters near high-traffic zones such as airports, metro stations, and other transit hubs (in addition to the existing train service). This would also require shortlisting high-margin, low-prep-time SKUs with selected partner restaurants and collaborating with ride-hailing platforms (e.g. Ola, Uber) to enable mid-route or pre-route pickups for scheduled cab/rental bookings, making it easier for daily travellers to grab their favourite food on the go and reducing long average delivery times (currently 55 minutes).
